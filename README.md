...
- 🌐 **网页提取**：`web_fetch` 工具获取 URL 正文，自动转为 AI 友好的 Markdown
  - 支持 `auto` / `http` / `js` 三种渲染模式
  - `auto` 模式自动检测 JS-only 页面并 fallback 到 Playwright 浏览器渲染
  - Apple Developer Documentation / Human Interface Guidelines 页面优先使用 Apple 原始 JSON 数据源（`/tutorials/data/...json`）提取正文，避免 JS 文档壳导致空内容或超时
  - 超时上限 12 分钟（`timeout_ms` 最大 720,000ms），适合重度 SPA 页面
- 🆓 **完全免费**：无需 API 密钥，无需付费
...
> 如果出现 `Executable doesn't exist`、`Please run playwright install` 等错误，说明 Python 依赖已安装但浏览器二进制缺失，需要重新执行上述安装命令。
...
- 🍎 Apple Developer Documentation / Human Interface Guidelines 自动使用 Apple 原始 JSON 数据源提取正文
...
> **提示**：`auto` 模式会先尝试轻量 HTTP 提取，检测到 JS-only 页面时自动切换到浏览器渲染，无需手动指定。Apple Developer Documentation / HIG 页面会优先走 Apple 原始 JSON fast path，不依赖 Playwright 渲染。
...
| `playwright` | Google/Bing/百度浏览器模式搜索与 `web_fetch` JS 渲染 |
| `pydantic` | 数据验证 |
...
- 🍎 **Apple 文档来源**：Apple Developer Documentation / HIG 页面使用 Apple 官方 `/tutorials/data/...json` 作为正文来源，并在返回结果中保留来源 URL
